...
## Code updated to use Phython (config.py) script 
## 
...
##/var/lib/ambari-server/resouces/scripts/configs.sh -u AmbariUserId -p AmbariPassword set AmbariServer Clustername  ranger-env ranger_group ux_ranger

##/var/lib/ambari-server/resources/scripts/configs.py -u admin -p admin -a set -l avm-1.field.hortonworks.com -n hdpf -c cluster-env -k user_group -v hadoop_hdpf
##/var/lib/ambari-server/resources/scripts/configs.py -u admin -p admin -a set -l avm-1.field.hortonworks.com -n hdpf -c livy2-env -k livy2_group -v hadoop_hdpf
##/var/lib/ambari-server/resources/scripts/configs.py -u admin -p admin -a set -l avm-1.field.hortonworks.com -n hdpf -c spark2-env -k spark_group -v hadoop_hdpf
##/var/lib/ambari-server/resources/scripts/configs.py -u admin -p admin -a set -l avm-1.field.hortonworks.com -n hdpf -c zeppelin-env -k zeppelin_group -v hadoop_hdpf
...
function change_user_name() {
      ...
      while read line
        do
            ...
            setuser=`echo "/var/lib/ambari-server/resources/scripts/configs.py -u $AMBARI_ADMIN_USERID -p $AMBARI_ADMIN_PASSWORD -a set  -l $AMBARI_SERVER -n ${CLUSTER_NAME} -c $envfile -k $newuservar -v $nuser"`
      ...
      while read line
        do
            ...
            /var/lib/ambari-server/resources/scripts/configs.py -u $AMBARI_ADMIN_USERID -p $AMBARI_ADMIN_PASSWORD -a get  -l $AMBARI_SERVER -n ${CLUSTER_NAME} -c $envfile |grep user\"
...
function get_user_name() {
        ...
        echo "Ambari Server is : " $AMBARI_SERVER
...
echo $envvars
 ...
 do
   ...
   echo $env
    ...
    userlist=`/var/lib/ambari-server/resources/scripts/configs.py -u $AMBARI_ADMIN_USERID -p $AMBARI_ADMIN_PASSWORD -a get  -l $AMBARI_SERVER -n ${CLUSTER_NAME} -c $env |grep user\" |grep ':'`
